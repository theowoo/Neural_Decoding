...
try:
    ...
    print(
        "\nWARNING: statsmodels is not installed. You will be unable to use the"
        "Naive Bayes Decoder"
    )
    pass
try:
    ...
    print(
        "\nWARNING: math is not installed. You will be unable to use the Naive Bayes"
        "Decoder"
    )
    pass
try:
    ...
    print(
        "\nWARNING: scipy is not installed. You will be unable to use the Naive Bayes"
        "Decoder"
    )
    pass
...
try:
    ...
    print(
        "\nWARNING: scikit-learn is not installed. You will be unable to use the Wiener"
        "Filter or Wiener Cascade Decoders"
    )
    pass
...
try:
    ...
    print(
        "\nWARNING: Xgboost package is not installed. You will be unable to use the"
        "xgboost decoder"
    )
    pass

# Import functions for Keras if Keras is installed
# Note that Keras has many more built-in functions that I have not imported because
# I have not used them
# But if you want to modify the decoders with other functions (e.g. regularization),
# import them here
try:
    import keras
    ...
    from keras.layers import GRU, LSTM, Activation, Dense, Dropout, SimpleRNN
...
try:
    ...
    print(
        "\nWARNING: Sklearn OneHotEncoder not installed. You will be unable to use"
...
class WienerCascadeRegression(object):
    ...
    def predict(self, X_flat_test):
        ...
        num_outputs = len(
            self.model
        )  # Number of outputs being predicted. Recall from the "fit" function
        # that self.model is a list of models
        y_test_predicted = np.empty(
            [X_flat_test.shape[0], num_outputs]
        )  # Initialize matrix that contains predicted outputs
        for i in range(num_outputs):  # Loop through outputs
            [regr, p] = self.model[
                i
            ]  # Get the linear (regr) and nonlinear (p) portions of the trained model
            # Predictions on test set
            y_test_predicted_linear = regr.predict(
                X_flat_test
            )  # Get predictions on the linear portion of the model
            y_test_predicted[:, i] = np.polyval(
                p, y_test_predicted_linear
            )  # Run the linear predictions through the nonlinearity to get the final
            # predictions
        return y_test_predicted
...
class KalmanFilterRegression(object):
    ...
    of Wu et al 2003 (https://papers.nips.cc/paper/2178-neural-decoding-of-cursor
    -motion-using-a-kalman-filter.pdf)
    ...
    def fit(self, X_kf_train, y_train):
        ...
        # First we'll rename and reformat the variables to be in a more standard kalman
        # filter nomenclature (specifically that from Wu et al, 2003):
        ...
        # Calculate the transition matrix (from x_t to x_t+1) using least-squares, and
        ...
        W = (
            (X2 - A * X1) * (X2 - A * X1).T / (nt - 1) / self.C
        )  # Covariance of transition matrix. Note we divide by nt-1 since only nt-1
        # points were used in the computation (that's the length of X1 and X2).
        # We also introduce the extra parameter C here.
    ...
    def predict(self, X_kf_test, y_test):
        ...
        # First we'll rename and reformat the variables to be in a more standard kalman
        # filter nomenclature (specifically that from Wu et al):
        # xs are the state (here, the variable we're predicting, i.e. y_train)
        # zs are the observed variable (neural data here, i.e. X_kf_train)
        ...
        states = np.empty(
            X.shape
        )  # Keep track of states over time (states is what will be returned as
        # y_test_predicted)
...
class DenseNNRegression(object):
    ...
    units: integer or vector of integers, optional, default 400
        ...
        If you want a single layer, input an integer (e.g. units=400 will give you
        a single hidden layer with 400 units)
        If you want multiple layers, input a vector (e.g. units=[400,200]) will give
        you 2 hidden layers with 400 and 200 units, repsectively.
    ...
    def __init__(self, units=400, dropout=0, num_epochs=10, verbose=0):
        ...
        try:  # Check if it's a vector
            ...
            TypeError
        ):  # If it's not a vector, create a vector of the number of units
            # for each layer
            units = [units]
        self.units = units
    ...
    def fit(self, X_flat_train, y_train):
        ...
        model.add(
            Dense(self.units[0], input_dim=X_flat_train.shape[1])
        )  # Add dense layer
        ...
        )  # Add final dense layer (connected to outputs)
        ...
        if keras_v1:
            model.fit(
                X_flat_train, y_train, nb_epoch=self.num_epochs, verbose=self.verbose
            )  # Fit the model
        else:
            model.fit(
                X_flat_train, y_train, epochs=self.num_epochs, verbose=self.verbose
            )  # Fit the model
        self.model = model
...
class XGBoostRegression(object):
    ...
    def fit(self, X_flat_train, y_train):
        ...
        for y_idx in range(num_outputs):  # Loop through outputs
            dtrain = xgb.DMatrix(
                X_flat_train, label=y_train[:, y_idx]
    ...
    def predict(self, X_flat_test):
        ...
        y_test_predicted = np.empty(
            [X_flat_test.shape[0], num_outputs]
        )  # Initialize matrix of predicted outputs
        for y_idx in range(num_outputs):  # Loop through outputs
...
class NaiveBayesRegression(object):
    ...
    res:int, default=100
        ...
        This is the number of bins to divide the outputs into (going from minimum
        to maximum)
    ...
    def fit(self, X_b_train, y_train):
        ...
        # column x,
        # second column y). This is called "input_xy"
        xs = np.reshape(
            input_mat[0], [input_x_range.shape[0] * input_y_range.shape[0], 1]
        )
        ys = np.reshape(
            input_mat[1], [input_x_range.shape[0] * input_y_range.shape[0], 1]
        )
        input_xy = np.concatenate((xs, ys), axis=1)

        # If quadratic model:
        #   -make covariates have squared components and mixture of x and y
        #   -do same thing for "input_xy", which are the values for creating the
        # tuning curves
        ...
        tuning_all = np.zeros(
            [num_nrns, input_xy.shape[0]]
        )  # Matrix that stores tuning curves for all neurons
        ...
        for j in range(num_nrns):  # Neuron number
            ...
            if self.encoding_model == "quadratic":
                tuning = glm_run(
                    y_train_modified, X_b_train[:, j : j + 1], input_xy_modified
                )
        ...
        # Here we're calculating the standard deviation of the change in state
        # (velocity/acceleration) in the training set
        n = y_train.shape[0]
        dx = np.zeros([n - 1, 1])
        for i in range(n - 1):
            dx[i] = np.sqrt(
                (y_train[i + 1, 0] - y_train[i, 0]) ** 2
                + (y_train[i + 1, 1] - y_train[i, 1]) ** 2
            )  # Change in state across time steps
        std = np.sqrt(
            np.mean(dx**2)
        )  # dx is only positive. this gets approximate stdev of distribution
        # (if it was
    ...
    def predict(self, X_b_test, y_test):
        ...
        y_test: numpy 2d array of shape [n_samples,n_outputs]
            The actual outputs
            This parameter is necesary for the NaiveBayesDecoder  (unlike most other
            decoders)
            because the first value is nececessary for initialization
        ...
        dists = squareform(
            pdist(input_xy, "euclidean")
        )  # Distance between all states in "input_xy"
        # Probability of going from one state to the next, based on the above calculated
        # distances
        # The probability is calculated based on the distances coming from a Gaussian
        # with standard deviation of std
        prob_dists = norm.pdf(dists, 0, std)

        # Initializations
        loc_idx = np.argmin(
            cdist(y_test[0:1, :], input_xy)
        )  # The index of the first location
        ...
        for t in range(num_ts):
            rs = X_b_test[
                t, :
            ]  # Number of spikes at this time point (in the interval we've specified
            # including bins_before and bins_after)

            probs_total = np.ones(
                [tuning_all[0, :].shape[0]]
            )  # Vector that stores the probabilities of being in any state based on the
            # neural activity (does not include probabilities of going from one state to
            # the next)
            for j in range(num_nrns):  # Loop across neurons
                ...
                probs = (
                    np.exp(-lam) * lam**r / math.factorial(r)
                )  # Probability of the given neuron's spike count given tuning curve
                # (assuming poisson distribution)
                probs_total = np.copy(
                    probs_total * probs
                )  # Update the probability across neurons (probabilities are multiplied
                # across neurons due to the independence assumption)
            prob_dists_vec = np.copy(
                prob_dists[loc_idx, :]
            )  # Probability of going to all states from the previous state
            probs_final = (
                probs_total * prob_dists_vec
            )  # Get final probability (multiply probabilities based on spike count and
            # previous state)
            # probs_final=probs_total*prob_dists_vec*self.p_x #Get final probability
            # when
            # including p(x), i.e. prior about being in states, which we're not using
            loc_idx = np.argmax(
                probs_final
            )  # Get the index of the current state (that w/ the highest probability)
            y_test_predicted[t, :] = input_xy[
                loc_idx, :
...
class WienerFilterClassification(object):
    ...
    def fit(self, X_flat_train, y_train):
        ...
        self.model = linear_model.LogisticRegression(
            C=self.C, multi_class="auto"
        )  # Initialize linear regression model
        # else:
        # self.model=linear_model.LogisticRegression(penalty='none',solver='newton-cg')
        # Initialize linear regression model
        self.model.fit(X_flat_train, y_train)  # Train the model
...
class DenseNNClassification(object):
    ...
    units: integer or vector of integers, optional, default 400
        This is the number of hidden units in each layer
        If you want a single layer, input an integer (e.g. units=400 will give you a
        single hidden layer with 400 units)
        If you want multiple layers, input a vector (e.g. units=[400,200]) will give
        you 2 hidden layers with 400 and 200 units, repsectively.
        The vector can either be a list or an array
    ...
    def fit(self, X_flat_train, y_train):
        ...
        for layer in range(self.num_layers - 1):  # Loop through additional layers
            ...
            model.add(
                Activation("tanh")
            )  # Add nonlinear (tanh) activation - can also make relu
            if self.dropout != 0:
                model.add(
                    Dropout(self.dropout)
                )  # Dropout some units if proportion of dropout != 0
...
class XGBoostClassification(object):
    ...
    gpu: integer, optional, default=-1
        if the gpu version of xgboost is installed, this can be used to select which gpu
        to use
        for negative values (default), the gpu is not used
    """
    ...
    def fit(self, X_flat_train, y_train):
        ...
        param = {
            ...
            # 'eval_metric': "merror",
            "max_depth": self.max_depth,  # this is the only parameter we have set, it's
            # one of the way or regularizing
            "eta": self.eta,
