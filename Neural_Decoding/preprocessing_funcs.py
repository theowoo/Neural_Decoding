...
def bin_spikes(spike_times, dt, wdw_start, wdw_end):
    ...
    spike_times: an array of arrays
        an array of neurons. within each neuron's array is an array containing
        all the spike times of that neuron
...
def bin_output(outputs, output_times, dt, wdw_start, wdw_end, downsample_factor=1):
    ...
    Parameters
    ----------
    outputs: matrix of size "number of times the output was recorded" x "number of
    ...
    Returns
    -------
    outputs_binned: matrix of size "number of time bins" x "number of features in
        the output"
    ...
    if downsample_factor != 1:  # Don't downsample if downsample_factor=1
        downsample_idxs = np.arange(
            0, output_times.shape[0], downsample_factor
        )  # Get the idxs of values we are going to include after downsampling
    ...
    for i in range(num_bins):  # Loop through bins
        idxs = np.where(
            ...
            0
        ]  # Indices to consider the output signal (when it's in the correct time range)
...
def get_spikes_with_history(neural_data, bins_before, bins_after, bins_current=1):
    ...
    Returns
    -------
    X: a matrix of size "number of total time bins" x "number of surrounding time
        bins used for prediction" x "number of neurons"
    ...
    # filled with NaNs, since they don't get filled in below.
    # This is because, for example, we cannot collect 10 time bins of spikes
    # before time bin 8
    start_idx = 0
    for i in range(
        num_examples - bins_before - bins_after
    ):  # The first bins_before and last bins_after bins don't get filled in
        end_idx = start_idx + surrounding_bins
        # The bins of neural data we will be including are between start_idx and end_idx
        # (which will have length "surrounding_bins")
        X[i + bins_before, :, :] = neural_data[
            start_idx:end_idx, :
